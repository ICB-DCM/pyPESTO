...
def add_candidate_to_dfs(design_problem: DesignProblem, candidate: dict,
    ...
    index_for_time_in_df = design_problem.petab_problem.measurement_df \
        .columns.get_loc("time")
    ...
    if have_noise:
        index_for_sd_in_df = \
            design_problem.petab_problem.measurement_df.columns.get_loc(
                "noiseParameters")
        ...
        if isinstance(noise, float):
            ...
            # how to make connection between the name of the parameter and
            # the internal name for the noise
            # here we would need something like "noiseParameter1_observable"
            ...
            raise Exception(
                "noise in measurement table has to be float or the name of "
    ...
    index_for_obs_id = design_problem.petab_problem.measurement_df.columns \
        .get_loc("observableId")
    observable_str = candidate['measurement_df'][index_for_obs_id]
    observable_index = design_problem.model.getObservableIds().index(
        observable_str)

    # exp_data.getObservedData is a long list of all observables at all time
    # points, hence the indexing
    ...
    measurement_df.at[
        len(measurement_df) - 1, 'measurement'] = measurement_with_noise
...
def delete_candidate_from_dfs(design_problem: DesignProblem, candidate: dict):
    ...
    index_sim_cond = design_problem.petab_problem.measurement_df.columns \
        .get_loc("simulationConditionId")
    measurement_df = design_problem.petab_problem.measurement_df
    test = measurement_df['simulationConditionId'][
        measurement_df['simulationConditionId'] == candidate['measurement_df'][
            index_sim_cond]].index.tolist()
