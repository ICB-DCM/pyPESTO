...
class InnerCalculatorCollector(AmiciCalculator):
    ...
    def _combine_inner_results(
        ...
        call_kwargs:
            The arguments this calculator was called with, forwarded to the
            inner calculators unchanged.
        index_slices:
            Passed on to :func:`calculate_quantitative_result`. ``None``
            derives them from the PEtab v1 parameter mapping.
        """
        sensi_orders = call_kwargs["sensi_orders"]
        x_ids = call_kwargs["x_ids"]
        dim = len(x_ids)

        nllh, snllh, s2nllh, chi2, res, sres = init_return_values(
            sensi_orders, call_kwargs["mode"], dim
        )
        interpretable_inner_pars = []
        spline_knots = None

        for calculator in self.inner_calculators:
            inner_result = calculator(rdatas=rdatas, **call_kwargs)
        ...
        if self.quantitative_data_mask is not None:
            quantitative_result = calculate_quantitative_result(
                ...
                edatas=call_kwargs["edatas"],
                mode=call_kwargs["mode"],
                quantitative_data_mask=self.quantitative_data_mask,
                dim=dim,
                parameter_mapping=call_kwargs["parameter_mapping"],
                par_opt_ids=x_ids,
                par_sim_ids=call_kwargs[
                    "amici_model"
    ...
    def __call__(
        ...
        return self._combine_inner_results(rdatas, call_kwargs)
