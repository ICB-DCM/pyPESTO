...
def waterfall(
    ...
    order_by_id:
        ...
        the same x-axis position. Only applicable when a list of results
        objects are provided. Default behavior is to sort the function values
        of each result independently.
...
def waterfall_lowlevel(
    ...
    start_indices = list(range(len(fvals)))
...
def get_ordering_by_start_id(results: Sequence[Result]) -> List[int]:
    ...
    The ordering is the generated by taking the best function value for each
    start across all results, then sorted these best function values.
