...
class AmiciObjectiveTest(unittest.TestCase):
    def runTest(self):
        for example in ["conversion_reaction"]:
            ...
            x0 = np.array(list(model.get_free_parameters()))
            ...
            assert np.all(
                (df.rel_err.values < RTOL) | (df.abs_err.values < ATOL)
            )
            ...
            for library in optimizers.keys():
                for method in optimizers[library]:
                    for fp in [[], [1]]:
                        with self.subTest(
                            library=library, solver=method, fp=fp
                        ):
...
def parameter_estimation(objective, library, solver, fixed_pars, n_starts):
    ...
    lb = -2 * np.ones((1, dim))
    ub = 2 * np.ones((1, dim))
    pars = objective.amici_model.get_free_parameters()
