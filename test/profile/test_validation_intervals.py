...
class ValidationIntervalTest(unittest.TestCase):
    ...
    def setUp(cls):
        ...
        cls.result_training_data = optimize.minimize(
            cls.problem_training_data, n_starts=5
        )
        # Optimum f(1)=2
        cls.result_all_data = optimize.minimize(
            cls.problem_all_data, n_starts=5
        )
    ...
    def test_validation_intervals(self):
        ...
        profile.validation_profile_significance(
        ...
        res_with_significance_true = profile.validation_profile_significance(
            self.problem_all_data, self.result_training_data
        )
        ...
        problem_val = pypesto.Problem(
            pypesto.objective.AggregatedObjective(
                [lsq_residual_objective(0), lsq_residual_objective(0)]
            ),
            self.lb,
            self.ub,
        )
        ...
        significance = profile.validation_profile_significance(
            problem_val, self.result_training_data
        )
        ...
        for d_val in [0.1, 1, 10, 100]:
            ...
            problem_val = pypesto.Problem(
                pypesto.objective.AggregatedObjective(
                    [lsq_residual_objective(0), lsq_residual_objective(d_val)]
                ),
                self.lb,
                self.ub,
            )
