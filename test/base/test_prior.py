...
def test_mode(scale, prior_type_list):
    ...
    prior_list = [get_parameter_prior_dict(iprior, prior_type, [1, 1], scale)
    ...
    true_optim = np.asarray(true_optim)
    ...
    for method in ['L-BFGS-B', 'ls_trf']:
        ...
        num_optim = result.optimize_result.list[0]['x'][~np.isnan(true_optim)]
        if num_optim.size:
            assert np.isclose(num_optim, true_optim[~np.isnan(true_optim)],
                              atol=1e-03).all()
...
def test_derivatives(prior_type_list, scale):
    ...
    # and in log scale...
    x0 = np.array([0.5] * len(prior_list))
