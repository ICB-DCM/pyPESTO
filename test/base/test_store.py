...
def test_storage_sampling():
    ...
    sample_original = sample.sample(
        problem=problem, sampler=sampler, n_samples=100, x0=[x_0]
    )
...
def test_storage_all():
    ...
    result = profile.parameter_profile(
        problem=problem, result=result, profile_index=[0], optimizer=optimizer
    )
    # Sampling

    sampler = sample.AdaptiveMetropolisSampler()
    result = sample.sample(
        problem=problem, sampler=sampler, n_samples=100, result=result
    )
    # Read and write
...
def test_result_from_hdf5_history():
    ...
    with tempfile.TemporaryDirectory(dir=".") as tmpdirname:
        ...
        # optimize with history saved to hdf5
        result = pypesto.optimize.minimize(
            problem=problem, n_starts=1, history_options=history_options_hdf5
        )
